...
// Calibration LED indicator circles (filled when corresponding LED is lit).
...
// Inference progress (0..100).
static uint8_t ui_inference_prog_bar = 0;
...
// Gaze point (normalized 0..255 for x/y)
static uint8_t ui_gaze_x = 128;
static uint8_t ui_gaze_y = 128;
...
template <> inline void ui_set<String>(UiDynamicVar var, const String &value) {
  switch (var) {
    ...
    case UiDynamicVar::UI_TEXT_EL_194: ui_text_el_194 = value; break;
    case UiDynamicVar::UI_CALIB_START_BTN: ui_calib_start_btn = value; break;
    case UiDynamicVar::UI_CALIB_NEXT_BTN: ui_calib_next_btn = value; break;
    case UiDynamicVar::UI_CALIB_REDO_BTN: ui_calib_redo_btn = value; break;
    case UiDynamicVar::UI_CALIB_RESULT: ui_calib_result = value; break;
    ...
    case UiDynamicVar::UI_CLOSE_EVENT_WARNING: ui_close_event_warning = value; break;
    case UiDynamicVar::UI_INFERENCE_TIMER: ui_inference_timer = value; break;
    case UiDynamicVar::UI_RESULTS_TITLE: ui_results_title = value; break;
    case UiDynamicVar::UI_RESULTS_NEXT_BTN: ui_results_next_btn = value; break;
    case UiDynamicVar::UI_RESULT_1: ui_result_1 = value; break;
    case UiDynamicVar::UI_RESULT_2: ui_result_2 = value; break;
    case UiDynamicVar::UI_RESULT_3: ui_result_3 = value; break;
    case UiDynamicVar::UI_RESULT_4: ui_result_4 = value; break;
    case UiDynamicVar::UI_RESULTS_PREV_BTN: ui_results_prev_btn = value; break;
    case UiDynamicVar::UI_TEXT_EL_269: ui_text_el_269 = value; break;
    default: break;
  }
}
...
inline void draw_calibration_screen(Adafruit_SSD1306 &display) {
  ...
  // LED corner indicators (filled if corresponding LED is active)
  display.drawCircle(7, 7, 5, SSD1306_WHITE);
  if (ui_led_up_left) display.fillCircle(7, 7, 3, SSD1306_WHITE);
  display.drawCircle(120, 7, 5, SSD1306_WHITE);
  if (ui_led_up_right) display.fillCircle(120, 7, 3, SSD1306_WHITE);
  display.drawCircle(8, 45, 5, SSD1306_WHITE);
  if (ui_led_bottom_left) display.fillCircle(8, 45, 3, SSD1306_WHITE);
  display.drawCircle(120, 45, 5, SSD1306_WHITE);
  if (ui_led_bottom_right) display.fillCircle(120, 45, 3, SSD1306_WHITE);
  display.setTextSize(1);
  display.setTextWrap(false);
  display.setTextColor(SSD1306_WHITE);
  {
...
inline void draw_recording_screen(Adafruit_SSD1306 &display) {
  ...
  display.setTextColor(SSD1306_WHITE);
  {
    ...
    while (i <= ui_event_name.length()) {
      ...
      const int x0 = 31 + max(0, (77 - (cols * 6 * 1)) / 2);
      display.setCursor(x0, cy);
      // Write this line.
  ...
  display.drawCircle(23, 45, 6, SSD1306_WHITE);
  display.setTextSize(1);
  display.setTextWrap(false);
  display.setTextColor(SSD1306_WHITE);
...
inline void draw_inference_loading_screen(Adafruit_SSD1306 &display) {
  ...
  {
    uint8_t p = ui_inference_prog_bar;
    if (p > 100) p = 100;
    int fillW = (int)((107L * p) / 100L); // interior width (109 - 2)
    if (fillW > 0) display.fillRect(11, 29, fillW, 3, SSD1306_WHITE);
  }
  display.setTextSize(1);
  display.setTextWrap(false);
  display.setTextColor(SSD1306_WHITE);
...
inline void draw_monitoring_screen(Adafruit_SSD1306 &display) {
  ...
  // Gaze point inside monitoring rect; ui_gaze_{x,y} are normalized 0..255
  {
    const int rx = 57, ry = 15, rw = 69, rh = 46;
    const int ix = rx + 2;
    const int iy = ry + 2;
    const int iw = rw - 4;
    const int ih = rh - 4;
    int gx = ix + (int)((uint16_t)ui_gaze_x * (uint16_t)(iw - 1) / 255U);
    int gy = iy + (int)((uint16_t)ui_gaze_y * (uint16_t)(ih - 1) / 255U);
    display.fillCircle(gx, gy, 2, SSD1306_WHITE);
  }
  display.drawCircle(15, 32, 8, SSD1306_WHITE);
  display.drawCircle(39, 32, 8, SSD1306_WHITE);
  // Eye validity indicators (check/cross)
  if (ui_left_eye) {
    display.drawLine(10, 32, 13, 36, SSD1306_WHITE);
    display.drawLine(13, 36, 19, 28, SSD1306_WHITE);
  } else {
    display.drawLine(10, 27, 19, 36, SSD1306_WHITE);
    display.drawLine(19, 27, 10, 36, SSD1306_WHITE);
  }
  if (ui_right_eye) {
    display.drawLine(34, 32, 37, 36, SSD1306_WHITE);
    display.drawLine(37, 36, 43, 28, SSD1306_WHITE);
  } else {
    display.drawLine(34, 27, 43, 36, SSD1306_WHITE);
    display.drawLine(43, 27, 34, 36, SSD1306_WHITE);
  }
  display.setTextSize(1);
  display.setTextWrap(false);
  display.setTextColor(SSD1306_WHITE);
